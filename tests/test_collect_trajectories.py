...
from train_w64 import SOURCE_DIMENSION, fit_pca_projection
...
def frozen_projection(tmp_path):
    ...
    hidden_states = rng.standard_normal((600, SOURCE_DIMENSION)).astype(np.float32)
...
def test_type_a_non_abstain_response_is_positive(collector):
    prompt = PromptRecord(
    ...
    generator = MockGenerator(min_tokens=25, max_tokens=30, hidden_dim=SOURCE_DIMENSION)
    ...
    assert record["label"] == Label.TYPE_A_POSITIVE.value
...
def test_short_sequence_is_class_e(collector):
    ...
    generator = MockGenerator(min_tokens=5, max_tokens=6, hidden_dim=SOURCE_DIMENSION)
...
def test_projection_uses_same_basis_across_layers(collector):
    ...
    h = np.random.default_rng(4).standard_normal((30, SOURCE_DIMENSION)).astype(np.float32)
