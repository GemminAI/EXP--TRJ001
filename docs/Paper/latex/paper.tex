...
\begin{abstract}
This study evaluates whether token-level hidden-state trajectories from selected layers of a large language model ({LLM}) can provide non-destructive signals for detecting response reliability and hedging/hallucination-related behavior~\citep{ji2023survey,azaria2023internal,farquhar2024semantic}. The evaluation instrument (Gram-determinant curvature $\kappa$, Gate~3 procedure, clustered bootstrap/{Holm} methodology) was pre-registered~\citep{nosek2018prereg,exptrj001planv13} for a different model (\texttt{meta-llama/Meta-Llama-3.1-8B-Instruct}, $D=4096$). Execution used \texttt{Qwen/Qwen2.5-7B-Instruct} ($D=3584$); the plan revision documenting that change~\citep{exptrj001plan} was written 2026-09-14, \emph{after} Phase~0--3 execution on 2026-09-13. Every result that depends on the model and dimension actually used is therefore reported as \emph{exploratory}, not confirmatory (Section~\ref{sec:prereg-status}).

All five Layer~16 representations---{PCA} $64${D} ($0.7806$), {PC4}--{67} ($0.7969$), $\bllen$ ($0.8077$), raw $3584${D} $\propk$ ($0.8472$), and $\blnorm$ ($0.8588$)---are compared as \emph{paired} evaluations on the same $98$-prompt test inventory ($n=496$ eligible generations). $\bllen$ and $\blnorm$ occupy the same $496$ rows as the trajectory methods. Under the {PCA} condition, $\propk$ failed the Gate~3 criterion for exceeding the response-length baseline ($\Delta=-0.0271$, $p=0.8685$; Gate~3: \textbf{fail}; exploratory). Removing compression recovered $\auroc=0.8472$ versus $\bllen$ ($\Delta=+0.0395$, $95\%$~{CI} $[0.0114, 0.0726]$, $p=0.004$; exploratory). Discarding {PC1}--{3} ({PC4}--{67}) raised {AUROC} only to $0.7969$ ($\Delta=-0.0108$, $p=0.6935$; $+0.0163$ vs.\ {PCA} $64${D}). The hidden-state $L_2$-norm baseline $\blnorm$ is the strongest single predictor ($0.8588$, $95\%$~{CI} $[0.8239, 0.8903]$).
...
Experiment~\exptag{} / {EXP-2026-NVS-001}. Primary model: \texttt{Qwen/Qwen2.5-7B-Instruct}~\citep{qwen25,yang2024qwen2}. Evaluated layers: $8$, $16$, $24$. Hidden dimension $D=3584$. Primary $64${D} {PCA} representation: \emph{exploratory, not confirmatory} (Section~\ref{sec:prereg-status}). Paired conditions on the same $98$-prompt test set ($n=496$): raw $3584${D} trajectory features, {PC4}--{67}, $\bllen$, and $\blnorm$.
...
The executed protocol used a $64$-dimensional {PCA} projection of $3584$-dimensional hidden states. This creates a controlled comparison, executed on one paired $98$-prompt test inventory, among
...
Can token-level hidden-state trajectories from the evaluated layers predict response reliability, and does their discriminative strength vary systematically across the evaluated layers? Intermediate-layer superiority is treated as a reconfirmation of a known probing pattern via trajectory curvature, not as a novel localization discovery.
...
\item \textbf{Transparent negative result (exploratory; Section~\ref{sec:prereg-status}).}
The $64${D} {PCA} condition did not satisfy Gate~3 versus $\bllen$. The failure is reported directly rather than being replaced by a post hoc successful configuration~\citep{simmons2011false,nosek2018prereg}. This is not a confirmatory pre-registered result for the model actually executed.
...
Let $\mathbf{z}_t \in \mathbb{R}^{D}$ denote the (possibly {PCA}-projected) hidden state at generated-token index $t\in\{1,\ldots,T\}$ and selected layer $\ell$, with $D=3584$ in raw space and $D=64$ after the $64${D} {PCA} projection.
...
\label{eq:kappa-cos}
\end{equation}
Equation~\eqref{eq:kappa-cos} is a \emph{scale-invariant directional} statistic: it is unchanged under any per-token rescaling $\mathbf{z}_t\mapsto \alpha_t\mathbf{z}_t$ with $\alpha_t>0$. Gram curvature~\eqref{eq:kappa-curv} is \emph{not} scale-invariant (it scales as inverse length). Reported $\propk$ is \emph{not} $\kappa^{\mathrm{cos}}$; it remains the frozen Gram estimator from the v1.3 instrument. $\kappa^{\mathrm{cos}}$ is the diagnostic that isolates turning from magnitude. Spearman correlation of \texttt{kappa\_mean} with mean $\lVert\mathbf{z}_t\rVert_2$, $\kappa^{\mathrm{cos}}$ {AUROC} on $L_2$-normalized trajectories, and the residual of $\kappa$ after regression on norm are the empirical checks of this distinction (\texttt{scripts/run\_trj001\_reanalysis.py}, Task~B).

\paragraph{Frozen Gram curvature.}
The reported $\propk$ scores use the discrete curvature specified in the v1.3 instrument~\citep{exptrj001planv13}, implemented in \texttt{src/compute\_kappa.py}. Parameterization is by token index, not arc length (an explicit, frozen choice: arc-length reparameterization would remove token-to-token speed variation). Central differences are
\begin{equation}
  \mathbf{v}_t
  \;=\;
...
The v1.3 feature set also includes \texttt{spike\_rate}$=(T-2)^{-1}\sum_t \mathbf{1}[\kappa_t>\theta]$, with $\theta$ the $90$th percentile of train-split $\kappa_t$. The raw $3584${D} ablation dropped \texttt{spike\_rate} because train-split raw states were unavailable to calibrate $\theta$; that condition difference is why {ABL-1} is additionally exploratory relative to the $6$-feature probe. A matched $5$-feature {PCA} $64${D} probe (same summaries, no \texttt{spike\_rate}) is the confounder control for feature cardinality and is computed by the reanalysis script (Task~D).
...
The frozen experimental environment records the configuration in Table~\ref{tab:env}. These values are recorded in the experimental freeze manifest~\citep{exptrj001freeze}. The model entry records the migration to {Qwen} from the {Llama} configuration named in v1.3~\citep{exptrj001planv13}. Inference used \texttt{vLLM}~\citep{kwon2023vllm}. The companion \texttt{configs/generation\_config.json} still names the pre-U8 {Llama} draft and is \emph{not} the execution environment.
...
\subsection{Pre-registration status of this execution}
\label{sec:prereg-status}

Plan v1.3~\citep{exptrj001planv13} pre-registers the evaluation instrument (Gram-determinant $\kappa$; Gate~3; clustered bootstrap/{Holm} methodology) for \texttt{meta-llama/Meta-Llama-3.1-8B-Instruct} ($D=4096$). The model actually executed, \texttt{Qwen/Qwen2.5-7B-Instruct} ($D=3584$), is a deviation from that pre-registration (update {U8}). A revised plan describing this and other deviations ({U1}, {U4}, {U8}, {U9})~\citep{exptrj001plan} exists only as of 2026-09-14---written \emph{after} Phase~0--3 execution (2026-09-13) and after the numeric results in this manuscript were already known.

Per the plan's own stated purpose (v1.3~\S1: freezing hypotheses and criteria before seeing data in order to prevent post-hoc reinterpretation and metric-selection bias), a plan revision written after the results are known cannot retroactively confer confirmatory status on those results, regardless of how faithfully the pre-specified statistical \emph{procedure} (bootstrap {CIs}, {Holm} correction, Gate~3's two-condition rule) was otherwise followed. Consequently:
\begin{itemize}[leftmargin=1.4em,itemsep=2pt]
\item Every result in this manuscript that depends on the {Qwen2.5-7B}/$3584${D} execution (the Gate~3 verdict and all {AUROC}/{CI}/$p$ values in Sections~\ref{sec:results}--\ref{sec:disc}) is \emph{exploratory}, not confirmatory---despite earlier drafts labeling some of them ``confirmatory'' or ``pre-registered.''
\item This does not mean the numbers are fabricated. The freeze record~\citep{exptrj001freeze} documents hash-verified artifacts and measured statistics. It means the numbers cannot claim the evidentiary weight of a true pre-registered confirmatory test (protection against $p$-hacking via advance commitment).
\item A genuine confirmatory test of {H0}/Gate~3 for the model actually pre-registered in v1.3 ({Llama-3.1-8B-Instruct}) has not been run.
\item The freeze manifest's own \texttt{v1.4-frozen} tag has not been applied (timestamp and commit fields remain blank), which is a separate, additional reason this data do not yet meet the plan's definition of a completed freeze.
\end{itemize}
...
\caption{Experimental pipeline. The $64${D} {PCA} branch (exploratory for this execution; Section~\ref{sec:prereg-status}) projects token-level hidden states with variance-maximizing {PCA} before $\kappa$-feature construction. The {ABL-1} branch derives the same curvature statistics from uncompressed $3584${D} states. Both branches, together with $\bllen$ and $\blnorm$, are scored by the same $L_2$-regularized logistic regression on the paired $98$-prompt test set.}
\label{fig:pipeline}
\end{figure}
...
\begin{enumerate}[leftmargin=1.4em,itemsep=2pt]
\item \textbf{{PCA} $64${D} (exploratory; Section~\ref{sec:prereg-status}).}
...
Response token length $T$ only. This is the Gate~3 comparator specified in the v1.3 instrument.
...
{PCA} diagnostics (Table~\ref{tab:pca-diag}) show that the $64${D} representation retains $99.992\%$ of global variance while still failing Gate~3. Explained variance is therefore a representation diagnostic, not evidence of task-signal preservation.
...
Because the executed model is a post-hoc deviation from v1.3 (Section~\ref{sec:prereg-status}), the study does \emph{not} maintain a confirmatory/exploratory split among the {Qwen} results: \emph{all} {AUROC}, $\Delta$, {CI}, and $p$ values that depend on this execution are exploratory. The v1.3 instrument (Gate~3 rule, clustered bootstrap, {Holm} correction) was specified in advance for a different model; that fact does not restore confirmatory status here.
...
The current uploaded manifest still shows the freeze timestamp, freeze commit, and approver fields as unfilled. The document also specifies that the freeze becomes formally established when all hash fields are complete and the \texttt{v1.4-frozen} tag has been applied. That unfilled tag is an additional reason, independent of Section~\ref{sec:prereg-status}, that these data do not yet meet the plan's definition of a completed freeze.

%======================================================================
\section{Results}
\label{sec:results}

\subsection{Gate~3 evaluation ({RQ2}) --- exploratory}
\label{sec:act1}

The primary {PCA} $64${D} $\propk$ evaluation at Layer~16 on the paired $98$-prompt test set is reported as exploratory (Section~\ref{sec:prereg-status}):
\begin{equation}
  \auroc_{\propk}=0.7806,
  \qquad
  \auroc_{\bllen}=0.8077,
...
\label{eq:gate3}
\end{equation}
This negative result does \emph{not} imply that hidden-state trajectories contain no useful information; it indicates only that the evaluated {PCA} trajectory representation did not beat $\bllen$ under the Gate~3 criterion, applied here in an exploratory (post-hoc-plan) capacity rather than as a confirmatory pre-registered test (Section~\ref{sec:prereg-status}).

\subsection{Layer localization ({RQ1})}
\label{sec:act2}

$\propk$ {AUROC} under the $64${D} {PCA} representation varies across the three sampled layers (Table~\ref{tab:layer}, Figure~\ref{fig:layer}): $0.7090$ at Layer~8, $0.7806$ at Layer~16, and $0.7167$ at Layer~24.
...
\begin{table}[t]
\centering
\caption{Layer-wise $\propk$ test {AUROC} ($64${D} {PCA}; exploratory; Section~\ref{sec:prereg-status}) with $95\%$ bootstrap confidence intervals.}
\label{tab:layer}
\small
\begin{tabular}{@{}crr@{}}
\toprule
...
\midrule
{PCA} $64${D} (exploratory; Section~\ref{sec:prereg-status}) & $0.7806$ & $-0.0271$ & $0.8685$ & $[0.7298,\ 0.8232]$ \\
...
\caption{Paired Layer~16 test {AUROC} on the $98$-prompt set ($n=496$): {PCA} $64${D} ($0.7806$; exploratory), {PC4}--{67} ($0.7969$), $\bllen$ ($0.8077$), raw $3584${D} ($0.8472$; exploratory), and $\blnorm$ ($0.8588$). $\blnorm$ and {PC4}--{67} are reported in full. All {Qwen} results are exploratory (Section~\ref{sec:prereg-status}).}
\label{fig:hero}
\end{figure}

\paragraph{Raw $3584${D}.}
Removing {PCA} yields exploratory $\auroc=0.8472$ ($\Delta=+0.0395$ vs.\ $\bllen$, $p=0.004$). This is not a Gate~3 success, confirmatory or otherwise.
...
\subsubsection{Exploratory status of the executed model}
All results that depend on the executed model are exploratory, not confirmatory (Section~\ref{sec:prereg-status}). The plan revision to \texttt{Qwen/Qwen2.5-7B-Instruct}/$3584${D}~\citep{exptrj001plan} was written after Phase~0--3 execution and after the numeric results were known; it cannot satisfy the pre-registration guarantee described in v1.3~\S1~\citep{exptrj001planv13}. This is the most significant limitation of the study as currently reported and is not merely a documentation formality.

\subsubsection{Exploratory status of {ABL-1}}
Raw $3584${D} $\propk$ was obtained after the $64${D} {PCA} outcome and dropped \texttt{spike\_rate}; Task~D is the $5$-feature control.
...
Only Layers~$8$, $16$, and $24$ were evaluated. Layer-$16$ superiority is a reconfirmation within that set.
...
Results are for \texttt{Qwen/Qwen2.5-7B-Instruct} only, which is itself a deviation from the model actually pre-registered before any data existed (\texttt{meta-llama/Meta-Llama-3.1-8B-Instruct}, v1.3).
...
\subsubsection{Incomplete freeze tag}
The freeze manifest's \texttt{v1.4-frozen} tag has not been applied (freeze date and commit fields remain blank), a separate gap from the post-hoc plan-document issue above.
...
Experiment~\exptag{} shows that variance-maximizing {PCA} can obscure task-critical trajectory signals because massive activations monopolize global variance.
The $64${D} {PCA} condition failed Gate~3 versus $\bllen$ ($\auroc_{\mathrm{PCA64}}=0.7806<\auroc_{\bllen}=0.8077$, $\Delta=-0.0271$, $p=0.8685$); this is an exploratory result for the executed model, \texttt{Qwen/Qwen2.5-7B-Instruct} (Section~\ref{sec:prereg-status}). A confirmatory test of the originally pre-registered model, {Llama-3.1-8B-Instruct}, has not been run.
On the same $98$-prompt test set ($n=496$), {PC4}--{67} reached only $0.7969$ ($+0.0163$ vs.\ {PCA} $64${D}), raw $3584${D} $\propk$ reached $0.8472$ ($p=0.004$ vs.\ $\bllen$; exploratory), and $\blnorm$ reached $0.8588$.
...
The findings \emph{indicate}---they do not establish as a universal law---that variance-preserving reduction is not signal-preserving reduction for this trajectory probe. The resulting scientific interpretation is deliberately narrower than a claim that {PCA} is universally harmful:
\begin{quote}
...
All {AUROC}, $\Delta$, {CI}, and $p$ values that depend on the {Qwen2.5-7B}/$3584${D} execution are exploratory (Section~\ref{sec:prereg-status}); massive-activation dominance of {PC1}--{3} is a mechanistic hypothesis~\citep{nosek2018prereg,simmons2011false,sun2024massive}.
...
\begin{table}[t]
\centering
\caption{Phase~3 exploratory {PCA} $64${D} and ablation outcomes (Layer~16 unless noted).}
\label{tab:phase3}
\small
\begin{tabular}{@{}ll@{}}
\toprule
